      ...
      <motion.header
        className={cn(
          ...
          scrollStatus.isInView
            ? "relative"
            : cn(
                "fixed -translate-y-full backdrop-blur-xs",
          ...
          <AnimatePresence>
            {open && (
              <motion.ul
                ...
                className={cn(
                  "absolute -mx-4 flex w-full flex-col overflow-hidden border-white/20 border-t backdrop-blur-xs",
                  mode === "dark" ? "bg-black/80" : "bg-white/80",
                )}
              >
                {links.map((link) => {
